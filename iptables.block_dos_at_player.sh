...
#    iptables -v -I DOCKER -p tcp --dport $PORT ! -s $DOCKERNET -m recent -i eth0 --update --seconds 60 --hitcount 10 --rttl -j DROP
    iptables -v -A DOCKER -p tcp --dport $PORT ! -s $DOCKERNET -m recent -i eth0 --update --seconds 60 --hitcount 10 --rttl --name BLOCKDOS -j DROP
    iptables -v -A DOCKER -p tcp --dport $PORT -m state --state NEW -m recent -i eth0 --set --name BLOCKDOS -j ACCEPT
done
exit
